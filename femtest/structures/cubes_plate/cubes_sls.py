...
FACES_NUM = 12
GUI = True
CONSTRAINT_FIXED_FACES = [92, 188, 284, 380, 476, 572, 668, 764, 749, 737, 725, 713, 701, 689, 677, 683, 587, 491, 395, 299, 203, 107, 11, 2, 14, 26, 38, 50, 62, 74, 86] # 278
VERTEX_UNDER_PRESSURE = [282, 290, 218]  # vertex

FRICTION_ACTIVATE = False

pressures_N_force = [i*1e7 / (10) for i in range(5, 7)]  # потому что прилагаются 3 силы к 3 вершинам
results = []
...
for curr_i, force in enumerate(pressures_N_force, start=1):
    ...
    import Mesh  # [Mesh module: https://wiki.freecad.org/Mesh]
    stl_file_path = "/home/ubnps23/tecHub/SLS_dev/CAD_testing/cad_analysis/planes_cmp/stlgen/cubes_layer_bs0.stl"  # Замените на актуальный путь
    # stl_file_path = "/home/ubnps23/tecHub/SLS_dev/CAD_testing/cad_analysis/planes_cmp/stlgen/csbs_0.stl"
    ...
    import Part
    shape = Part.Shape()
    tolerance = 0.1  # Допуск аппроксимации (подберите опытным путем)
    ...
    # 10. Фиксируем нижние поверхности многогранников
    # Определяем список имен граней для фиксации
    ...
    # Добавляем ограничение в анализ
    ...
    filename = '/home/ubnps23/tecHub/SLS_dev/CAD_testing/cad_analysis/planes_cmp/cc_pairs.csv'
        ...
        #     for j in range(0):
                ...
                for idxA, faceA in enumerate(A.Shape.Faces, start=1):
                    for idxB, faceB in enumerate(B.Shape.Faces, start=1):
                        ...
                        if (centerA - centerB).Length < 1.5e2:
    ...
    base_force = force / len(VERTEX_UNDER_PRESSURE) # базовая величина силы

    faces_under_pressure_idx = VERTEX_UNDER_PRESSURE
    # faces_under_pressure_idx = [443, 421, 325]  # faces
    # faces_under_pressure_idx = list(range(100, 131))
    for i in faces_under_pressure_idx:
        face_name = f"Vertex{i}"
        # face_name = f"Face{i}"
        try:
            force_constraint = ObjectsFem.makeConstraintForce(doc, f"Force_{face_name}")
            force_constraint.References = [(cmp_obj, face_name)]
            ...
            direction_obj = doc.addObject("Part::Feature", f"Direction_{face_name}")
            ...
            print(f"Ошибка при создании силового ограничения для {face_name}: {e}")
